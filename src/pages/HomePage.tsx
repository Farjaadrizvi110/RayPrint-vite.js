  ...
  return (
    <section ref={ref} className="relative min-h-screen flex items-center overflow-hidden">
        ...
        <div className="max-w-3xl" style={{ perspective: '1000px' }}>
          ...
          <div className="flex flex-wrap gap-4 mb-16">
            ...
            <Link to="/products">
              <Button
                variant="outline"
                className="hero-btn group border-[#3B6CFF]/30 bg-blue-50 text-[#3B6CFF] hover:bg-blue-100 hover:border-[#3B6CFF]/60 text-lg px-8 py-7 rounded-2xl font-semibold transition-all duration-300 hover:scale-105 shadow-sm"
              >
      ...
      >
        ...
        >
          <span className="text-xs text-[#64748B]">Scroll to explore</span>
          <div className="w-6 h-10 rounded-full border-2 border-[rgba(15,23,42,0.25)] flex justify-center pt-2">
            <motion.div 
    ...
    >
      ...
      <div className="rp-container">
        <div className="text-center mb-16" style={{ perspective: '1000px' }}>
          ...
          <h2 className="category-title text-5xl md:text-6xl font-bold text-[#0F172A] mb-6">
          ...
          {categories.slice(0, 11).map((category) => (
            <Link key={category.id} to={`/products/${category.slug}`}>
              ...
              >
                ...
                {/* Card content */}
                <div className="relative h-full bg-white border border-slate-200 rounded-3xl overflow-hidden group-hover:border-[rgba(59,108,255,0.5)] shadow-sm group-hover:shadow-xl transition-all duration-500">
                  <img
                    src={category.image}
                    alt={category.name}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-base md:text-lg font-semibold text-white group-hover:text-[#93c5fd] transition-colors">
                          {category.name}
                        </h3>
                        <p className="text-xs text-white/70 mt-1">{category.productCount} products</p>
                      </div>
        ...
        <div className={`grid lg:grid-cols-2 gap-12 lg:gap-20 items-center ${isReversed ? 'lg:flex-row-reverse' : ''}`}>
          ...
          <div className={`product-content-${index} ${isReversed ? 'lg:order-2' : ''}`}>
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              {product.specs.map((spec, i) => (
                <span
                  key={i}
                  className="px-4 py-2 bg-slate-100 border border-slate-200 rounded-full text-sm text-[#475569] hover:border-[rgba(59,108,255,0.5)] hover:text-[#3B6CFF] hover:bg-[rgba(59,108,255,0.05)] transition-all"
                >
                  {spec}
                </span>
              ))}
          ...
          >
            ...
            >
              ...
              {/* Floating badge */}
              <motion.div
                className="absolute bottom-6 left-6 right-6 p-4 bg-white/90 backdrop-blur-xl border border-white/80 rounded-2xl shadow-lg"
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-[#0F172A]">£{product.priceTiers[0]?.price.toFixed(2)}</p>
                  </div>
      ...
      <div className="rp-container relative">
        ...
        <div className="sustain-stats grid md:grid-cols-3 gap-6" style={{ perspective: '1000px' }}>
          {stats.map((stat, index) => (
            <motion.div
              key={index}
              className="sustain-stat group relative p-8 rounded-3xl bg-white border border-slate-200 hover:border-[rgba(59,108,255,0.4)] shadow-sm hover:shadow-xl transition-all duration-500"
      ...
      <div className="rp-container">
        ...
        <div className="reviews-grid grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((review) => (
            <motion.div
              key={review.id}
              className="review-card group p-8 rounded-3xl bg-white border border-slate-200 hover:border-[rgba(59,108,255,0.4)] shadow-sm hover:shadow-xl transition-all duration-500"
            ...
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-medium text-[#0F172A]">{review.userName}</p>
                  <p className="text-sm text-[#64748B]">{review.userRole}</p>
                </div>
              </div>
            </motion.div>
          ))}
