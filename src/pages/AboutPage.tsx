    ...
    <div className="min-h-screen bg-[#0B0F17] pt-32 pb-20">
      <div className="rp-container">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-20">
          ...
          <p className="text-lg text-[#A6B0C5] max-w-3xl mx-auto leading-relaxed">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-20">
          {stats.map((stat, index) => (
            <div key={index} className="rp-card p-6 text-center hover:border-[rgba(59,108,255,0.4)] transition-colors">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="grid lg:grid-cols-2 gap-12 items-center mb-20">
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-5 -left-5 bg-[#0B0F17] border border-[rgba(59,108,255,0.4)] rounded-2xl px-5 py-4 shadow-xl">
              <p className="text-xs text-[#A6B0C5] mb-1">Company Status</p>
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                <p className="text-[#F6F8FF] font-semibold">Active — Est. 2016</p>
              </div>
            </div>
          </div>
        </motion.div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {expertise.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}
                className="rp-card p-6 hover:border-[rgba(59,108,255,0.4)] transition-all group">
                <div className="w-12 h-12 rounded-xl bg-[rgba(59,108,255,0.15)] flex items-center justify-center mb-4 group-hover:bg-[rgba(59,108,255,0.25)] transition-colors">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {values.map((value, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.08 }}
                className="rp-card p-6 hover:border-[rgba(246,248,255,0.20)] transition-colors group">
                <value.icon className="w-8 h-8 text-[#3B6CFF] mb-4" />
                <h3 className="text-lg font-semibold text-[#F6F8FF] mb-2">{value.title}</h3>
                <p className="text-sm text-[#A6B0C5] leading-relaxed">{value.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-20">
          ...
          <div className="max-w-3xl mx-auto rp-card p-8 border-[rgba(59,108,255,0.25)]">
            <div className="flex items-center gap-3 mb-8 pb-6 border-b border-[rgba(246,248,255,0.08)]">
              <div className="w-12 h-12 rounded-xl bg-[rgba(59,108,255,0.15)] flex items-center justify-center">
                <Building2 className="w-6 h-6 text-[#3B6CFF]" />
              </div>
              <div>
                <p className="text-xl font-bold text-[#F6F8FF]">RAY DESIGN AND PRINT LTD</p>
                <p className="text-sm text-[#A6B0C5]">Company No. <span className="text-[#F6F8FF] font-semibold">10272806</span></p>
              </div>
              <div className="ml-auto flex items-center gap-2 px-3 py-1.5 rounded-full bg-[rgba(34,197,94,0.1)] border border-[rgba(34,197,94,0.25)]">
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-xs text-[#A6B0C5] mb-1 uppercase tracking-wider">Registered Address</p>
                  <p className="text-sm text-[#F6F8FF] font-medium leading-snug">1a Edmundson Street,<br />Blackburn, BB2 1HL<br />United Kingdom</p>
                </div>
              </div>
              <div className="flex gap-3">
                <CalendarCheck className="w-5 h-5 text-[#3B6CFF] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs text-[#A6B0C5] mb-1 uppercase tracking-wider">Incorporated On</p>
                  <p className="text-sm text-[#F6F8FF] font-medium">12 July 2016</p>
                </div>
              </div>
              <div className="flex gap-3">
                <ShieldCheck className="w-5 h-5 text-[#3B6CFF] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs text-[#A6B0C5] mb-1 uppercase tracking-wider">Company Type</p>
                  <p className="text-sm text-[#F6F8FF] font-medium">Private Limited Company</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
          className="text-center rp-card p-12 border-[rgba(59,108,255,0.25)] bg-[rgba(59,108,255,0.04)]">
          <h2 className="text-3xl md:text-4xl font-bold text-[#F6F8FF] mb-4">Ready to make an impression?</h2>
          <p className="text-[#A6B0C5] mb-8 max-w-xl mx-auto">Browse our full range of print products — or get in touch and let our team help you find the perfect solution for your brand.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/products">
              <Button className="bg-[#3B6CFF] hover:bg-[#2a5aee] text-white px-8 py-6 text-base">Browse Products</Button>
            </Link>
            <Link to="/contact">
              <Button variant="outline" className="border-[rgba(246,248,255,0.20)] text-[#F6F8FF] hover:bg-[rgba(246,248,255,0.05)] px-8 py-6 text-base">Get in Touch</Button>
            </Link>
          </div>
        </motion.div>
