...
const getOptionIcon = (name: string) => {
  ...
  if (l.includes('paper'))    return <FileText className="w-4 h-4" />;
  if (l.includes('finish'))   return <Info     className="w-4 h-4" />;
  if (l.includes('print'))    return <FileText className="w-4 h-4" />;
  if (l.includes('corner'))   return <Package  className="w-4 h-4" />;
...
export function BusinessCardPage() {
  ...
  const [selectedQuantity,   setSelectedQuantity]   = useState<number>(7); // index 7 = 500 qty
  const [uploadedArtwork,    setUploadedArtwork]    = useState<Artwork | null>(null);
  const [expandedOption,     setExpandedOption]     = useState<string | null>(null);
  const [isUploadingArtwork, setIsUploadingArtwork] = useState(false);

  // Set recommended defaults when product loads
  useEffect(() => {
    if (product) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res  = await fetch(`${BACKEND_URL}/api/artwork/upload`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: formData });
    ...
    <div className="min-h-screen bg-white pt-28 pb-20">
      ...
      <div className="rp-container">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-6">
          <Link to="/products/business-cards" className="group inline-flex items-center gap-2.5 px-4 py-2 rounded-xl bg-blue-50 border border-blue-200 text-sm font-medium text-[#3B6CFF] hover:text-[#2a5aee] hover:bg-blue-100 hover:border-blue-300 hover:shadow-lg hover:shadow-[#3B6CFF]/10 transition-all duration-300">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-10">
          <span className="rp-micro-label block mb-3">BUSINESS CARDS</span>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                {product.priceTiers.map((tier, index) => (
                  ...
                  >
                    ...
                    <p className="text-xs opacity-80">£{tier.price.toFixed(2)}</p>
                  </button>
                ))}
              </div>
            </div>
