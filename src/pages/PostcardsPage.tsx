    ...
    <div className="min-h-screen bg-white pt-28 pb-20">
      ...
      <div className="rp-container">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-6">
          <Link to="/products/postcards-flyers" className="group inline-flex items-center gap-2.5 px-4 py-2 rounded-xl bg-blue-50 border border-blue-200 text-sm font-medium text-[#3B6CFF] hover:text-[#2a5aee] hover:bg-blue-100 hover:border-blue-300 hover:shadow-lg hover:shadow-[#3B6CFF]/10 transition-all duration-300">
        ...
        <div className="grid xl:grid-cols-12 gap-8 lg:gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="xl:col-span-7">
                              ...
                              {option.values.map((value) => {
                                ...
                                return (
                                  <button key={value} onClick={() => { handleOptionChange(option.name, value); setExpandedOption(null); }}
                                    className={`relative px-3 py-3 rounded-xl text-sm text-left transition-all ${selectedOptions[option.name] === value ? 'bg-[#3B6CFF] text-white' : 'bg-[#F1F5F9] text-[#475569] hover:bg-[#E2E8F0] hover:text-[#0F172A]'} ${isRec ? 'pb-7' : ''}`}
                                  >
                                    <span>{value}</span>
                ...
                {product.priceTiers.map((tier, idx) => {
                  ...
                  return (
                    <button key={tier.quantity} onClick={() => setSelectedQtyIdx(idx)}
                      className={`relative flex items-center justify-between px-4 py-3 rounded-xl border text-sm transition-all ${isSelected ? 'border-[#3B6CFF] bg-blue-50' : 'border-[#E2E8F0] bg-[#F8FAFC] hover:border-[#CBD5E1]'} ${isRec ? 'mt-1' : ''}`}
                    >
                      ...
                      <span className={`font-medium ${isSelected ? 'text-[#3B6CFF]' : 'text-[#475569]'}`}>{tier.quantity.toLocaleString()}</span>
                      <span className={`font-semibold ${isSelected ? 'text-[#0F172A]' : 'text-[#475569]'}`}>£{tier.price.toFixed(2)}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="mb-8">
              ...
              {uploadedArtwork ? (
                <div className={`p-4 rounded-xl border flex items-center justify-between ${isUploadingArtwork ? 'bg-amber-50 border-amber-200' : 'bg-blue-50 border-blue-200'}`}>
                  <div className="flex items-center gap-3">
                  ...
                  {!isUploadingArtwork && <button onClick={() => setUploadedArtwork(null)} className="text-sm text-[#64748B] hover:text-red-500 transition-colors">Remove</button>}
                </div>
              ) : (
            ...
            <div className="p-6 rounded-2xl bg-[#F8FAFC] border border-[#E2E8F0]">
              <div className="flex items-end justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-[#64748B] mt-1">{currentTier?.quantity.toLocaleString()} postcards @ £{(totalPrice / (currentTier?.quantity || 1)).toFixed(3)} each</p>
                </div>
